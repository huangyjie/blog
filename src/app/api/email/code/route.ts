...
const transporter = nodemailer.createTransport({
  service: 'QQ',
  auth: {
    user: '1401466869@qq.com',
    pass: 'hdeusicgyclwjcai'
  },
  debug: true
})
...
// 保存验证码
async function saveCodes(codes: any) {
...
export async function POST(request: Request) {
  try {
    ...
    console.log('请求体:', { email, length, expire })
    ...
    // 使用全局 Map
    const existingCode = global.verificationCodes.get(email)
    
    // 检查发送频率
    if (existingCode) {
      const timeSinceLastSend = Date.now() - existingCode.createTime
      if (timeSinceLastSend < 60000) {
        console.log('发送太频繁:', email, '上次发送时间:', new Date(existingCode.createTime))
        return NextResponse.json({
          status: 'error',
          message: '发送太频繁，请稍后再试'
        })
    ...
    console.log('生成的验证码:', code, '邮箱:', email)
    ...
    try {
      ...
      const info = await transporter.sendMail({
        from: '"H\'s blog circle 验证服务" <1401466869@qq.com>',
        to: email,
